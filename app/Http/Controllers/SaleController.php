...
class SaleController extends Controller
{
    public function register(Request $request) {
        ...
        $var = function() {     
            $prg = func_get_arg(0);
            $stack = [];
            $importe_pagar_con_igv = 0.00;
            $importe_pagar_sin_igv = 0.00;
            $importe_pagar_igv = 0.00;

            foreach($prg as $item) {
                if ($item['descripcion'] !== "--"){
                    $carga = Carga::find($item['descripcion']);
                    if ($carga) {
                        // el importe proviene del formulario e incluye IGV
                        // $precio_con_igv = number_format($item['cantidad']*$item['importe']*$item['peso'], 2, '.', '');
                        
                        // el importe proviene del formulario y NO incluye IGV
                        $precio_unitario_con_igv = $item['importe']*1.18;
                        $precio_con_igv = number_format($item['cantidad']*$precio_unitario_con_igv*$item['peso'], 2, '.', '');
                        
                        $precio_sin_igv = number_format($precio_con_igv/1.18, 2, '.', '');
                        $precio_igv = number_format($precio_sin_igv*0.18, 2, '.', '');
                        $importe_pagar_con_igv += $precio_con_igv;
                        $importe_pagar_sin_igv += $precio_sin_igv;
                        $importe_pagar_igv += $precio_igv;
                        ...
                        array_push($stack, [
                            ...
                            // 'importe_pagar_con_igv' => $importe_pagar_con_igv,
                            // 'importe_pagar_sin_igv' => $importe_pagar_sin_igv,
                            // 'importe_pagar_igv' => $importe_pagar_igv,
                        ]);
                    }
                }
            }
        ...
        if ($documento->alias === 'B') {
            ...
            return \response()->json(['result' => ['status' => 'fails', 'message' => 'Ha ocurrido un error(100) en el sistema, contacta con el proveedor.']]);
        }
        ...
        if (strlen($data['encargo_id']) > 0) {
            ...
            // debes bloquear actualizar los registros
    ...
    public function show() {
        ...
        return view('sale.show')->with([ 'agencia_origen' => $agencia_origen, 'sede' => $sede, 'documento' => $documento, 'carga' => $carga ]);
    }
    ...
    public function escribirXMLFactura($encargo_id) {
        ...
        if ($data) {
            ...
            // begin 9 - tipo de documento (Factura) **
                ...
                $nodeInvoiceTypeCode = $dom->createElement('cbc:InvoiceTypeCode', $factura); // 01:Factura
                ...
                $root->appendChild($nodeAccountingCustomerParty);
                    ...
                    $nodeAccountingCustomerParty->appendChild($nodeParty);
                        ...
                        $nodeParty->appendChild($nodePartyTaxScheme);
                            ...
                            $nodeCompanyID = $dom->createElement('cbc:CompanyID', $data['adquiriente_ruc']);
